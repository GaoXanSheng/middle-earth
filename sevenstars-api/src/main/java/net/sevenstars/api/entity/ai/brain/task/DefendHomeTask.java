        ...
        if(optionalHome != null && optionalHome.isPresent()) {
            if(optionalPlayers != null && optionalPlayers.isPresent()) {
                for(Player player : optionalPlayers.get()) {
                    if(world.isDarkOutside() && player.isShiftKeyDown()) { // Don't trigger if player is sneaking
                        return null;
                    }
                    ...
                    if(player.position().distanceToSqr(Vec3.atCenterOf(optionalHome.get().pos())) < (radius*radius) && !player.hasInfiniteMaterials()) {
                        return player;
                    }
                }
            }
