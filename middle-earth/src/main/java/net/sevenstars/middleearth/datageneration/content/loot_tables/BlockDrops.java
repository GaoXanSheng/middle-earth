...
public class BlockDrops {
    // Blocks are registered into this list during class-init of the registration classes,
    // so the static catalog below must stay lazy: populating it in <clinit> would initialize
    // ModDecorativeBlocks/ModNatureBlocks while they are still initializing themselves.
    public static List<Block> blocks = new ArrayList<>();

    private static boolean catalogPopulated = false;

    public static void ensureCatalogPopulated() {
        if (catalogPopulated) {
            return;
        }
