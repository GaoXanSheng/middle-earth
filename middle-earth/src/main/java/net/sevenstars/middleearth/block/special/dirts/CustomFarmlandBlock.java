...
public class CustomFarmlandBlock extends FarmlandBlock {
    ...
    public void fallOn(Level world, BlockState state, BlockPos pos, Entity entity, double fallDistance) {
        if (world instanceof ServerLevel serverWorld) {
            if ((double)world.getRandom().nextFloat() < fallDistance - 0.5 && entity instanceof LivingEntity && (entity instanceof Player || serverWorld.getGameRules().get(GameRules.MOB_GRIEFING)) && entity.getBbWidth() * entity.getBbWidth() * entity.getBbHeight() > 0.512F) {
                setToDirt((Entity)null, state, world, pos, target);
            }
        }
