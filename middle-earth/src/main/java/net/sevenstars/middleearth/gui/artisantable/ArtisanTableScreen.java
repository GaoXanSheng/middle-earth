...
public class ArtisanTableScreen extends AbstractContainerScreen<ArtisanTableScreenHandler> implements ContainerListener {
    private static final Identifier TEXTURE = Identifier.fromNamespaceAndPath(MiddleEarth.MOD_ID, "textures/gui/artisan_table.png");
    ...
    public ArtisanTableScreen(ArtisanTableScreenHandler handler, Inventory inventory, Component title) {
        super(handler, inventory, title, 232, 166);
    ...
    public void equipArmorStand(ItemStack stack) {
        ...
        if (!stack.isEmpty()) {
            ...
            if (itemStack.getComponents().has(DataComponents.EQUIPPABLE)) {
                ...
                this.armorStand.setItemSlot(equipmentSlot, stack.copy());
            } else {
    ...
    public void extractBackground(GuiGraphicsExtractor context, int mouseX, int mouseY, float delta) {
        ...
        context.blit(RenderPipelines.GUI_TEXTURED, TEXTURE, i + 143, j + 15 + k, 232 + (this.shouldScroll() ? 0 : 12), 0, 12, 15, 256, 256);
        ...
        for(int w = 0; w < 3; w++) {
            for(int z = 0; z < 3; z++) {
                if(menu.slots.get(w*3 + z).isActive()) {
                    context.blit(RenderPipelines.GUI_TEXTURED, TEXTURE,leftPos + SLOT_OFFSET_X + SLOT_SCALE*z,topPos + SLOT_OFFSET_Y + SLOT_SCALE*w, 232, 15, SLOT_SCALE, SLOT_SCALE, 256, 256);
                    InputType inputType = shape.getInputType(z, w);
                    if(!menu.slots.get(w*3 + z).hasItem()) {
                        switch (inputType){
                            case HANDLE -> context.blit(RenderPipelines.GUI_TEXTURED, TEXTURE,leftPos + SPRITE_OFFSET_X + SLOT_SCALE*z,topPos + SPRITE_OFFSET_Y + SLOT_SCALE*w, 232, 87, 16, 16, 256, 256);
                            case HILT -> context.blit(RenderPipelines.GUI_TEXTURED, TEXTURE,leftPos + SPRITE_OFFSET_X + SLOT_SCALE*z,topPos + SPRITE_OFFSET_Y + SLOT_SCALE*w, 232, 103, 16, 16, 256, 256);
                            case BLADE -> context.blit(RenderPipelines.GUI_TEXTURED, TEXTURE,leftPos + SPRITE_OFFSET_X + SLOT_SCALE*z,topPos + SPRITE_OFFSET_Y + SLOT_SCALE*w, 232, 119, 16, 16, 256, 256);
                            case AXE -> context.blit(RenderPipelines.GUI_TEXTURED, TEXTURE,leftPos + SPRITE_OFFSET_X + SLOT_SCALE*z,topPos + SPRITE_OFFSET_Y + SLOT_SCALE*w, 232, 135, 16, 16, 256, 256);
                            case PICKAXE -> context.blit(RenderPipelines.GUI_TEXTURED, TEXTURE,leftPos + SPRITE_OFFSET_X + SLOT_SCALE*z,topPos + SPRITE_OFFSET_Y + SLOT_SCALE*w, 232, 151, 16, 16, 256, 256);
                            case SHOVEL -> context.blit(RenderPipelines.GUI_TEXTURED, TEXTURE,leftPos + SPRITE_OFFSET_X + SLOT_SCALE*z,topPos + SPRITE_OFFSET_Y + SLOT_SCALE*w, 232, 167, 16, 16, 256, 256);
                            case HOE -> context.blit(RenderPipelines.GUI_TEXTURED, TEXTURE,leftPos + SPRITE_OFFSET_X + SLOT_SCALE*z,topPos + SPRITE_OFFSET_Y + SLOT_SCALE*w, 232, 183, 16, 16, 256, 256);
                        }
                    }
                }
            }
    ...
    public void extractRenderState(GuiGraphicsExtractor context, int mouseX, int mouseY, float delta) {
        ...
        for (ArtisanTableTab category : this.categories) {
            ...
            context.setTooltipForNextFrame(this.font, category.getTitle(), mouseX, mouseY);
        }
        for (ArtisanTableTab tab : this.tabs.get(selectedCategory.getIndex())) {
        ...
        if (canCraft()) {
            ...
            for(int l = this.scrollOffset; l < k && l < (this.menu).getAvailableOutputs().size(); ++l) {
                ...
                if (x >= n && x < n + 16 && y >= o && y < o + 18) {
                    context.setTooltipForNextFrame(this.font, list.get(l), x, y);
                }
            }
        }
    }
