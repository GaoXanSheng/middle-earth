...
public class OnboardingFactionScreen extends Screen {
    private static final Identifier MAP_UI_IDENTIFIER = Identifier.fromNamespaceAndPath(MiddleEarth.MOD_ID,"textures/gui/faction_selection_map.png");
    private static final Identifier BUTTON_UI_IDENTIFIER = Identifier.fromNamespaceAndPath(MiddleEarth.MOD_ID,"textures/gui/faction_selection_buttons.png");
    ...
    public class OnboardingFactionScreenElements {
        ...
        public DrawablePanel informationPanel =
                new DrawablePanel(169, 207, 0, 0, Identifier.fromNamespaceAndPath(MiddleEarth.MOD_ID,"textures/gui/faction_selection.png"));
    ...
    private class DrawablePanel{
        ...
        public void draw(GuiGraphicsExtractor context, int startX, int startY){
            this.startX = startX;
            this.startY = startY;
            context.blit(RenderPipelines.GUI_TEXTURED, texture, startX, startY, uvX, uvY, width, height, 256, 256);
        }
    }
    ...
    protected void init() {
        ...
        elements.bannerField = this.minecraft.getEntityModels().bakeLayer(ModelLayers.STANDING_BANNER_FLAG).getChild("flag");
    ...
    private void renderDisplays(GuiGraphicsExtractor context, int mouseX, int mouseY, float delta) {
        ...
        context.text(font, elements.factionName, factionStartX, startY, TEXT_COLOR, false);
        if(isMouseOver(factionStartX, font.width(elements.factionName), startY, font.lineHeight)){
            context.setComponentTooltipForNextFrame(font, List.of(controller.getCurrentFactionFullName()), ModWidget.getMouseX(), ModWidget.getMouseY());
        }
        ...
        if(elements.subfactionName != null){
            ...
            context.text(font, subfactionTitle, startX, startY, TEXT_COLOR, false);
            context.text(font, elements.subfactionName, startX + font.width(subfactionTitle), startY, TEXT_COLOR, false);
        }
        // Race.s
        if(elements.raceList != null){
            ...
            context.text(minecraft.font, raceTitle, startX, startY, TEXT_COLOR, false);
            startY += font.lineHeight + 3;
        ...
        context.text(minecraft.font, Component.translatable("screen." + MiddleEarth.MOD_ID + ".information.description").withStyle(ChatFormatting.UNDERLINE),
        ...
        if(elements.bannerComponents != null && !elements.bannerComponents.isEmpty()) {
            var bannerPatternRegistry = this.minecraft.level.registryAccess().lookup(Registries.BANNER_PATTERN);
            ...
            for(BannerData.BannerPatternWithColor entry : elements.bannerComponents){
                ...
                Holder<BannerPattern> pattern = bannerPatternRegistry.get().wrapAsHolder(entry.pattern);
            ...
            context.guiRenderState.addPicturesInPictureState(new BannerResultWithScaleGuiElementRenderState(this.elements.bannerField, DyeColor.GRAY, bannerBuilder.build(), bannerX, 0, bannerX + 40, bannerY + 80, 32f, context.scissorStack.peek()));
        }
        ...
        context.blit(RenderPipelines.GUI_TEXTURED, MAP_UI_IDENTIFIER,
        ...
        context.blit(RenderPipelines.GUI_TEXTURED, BUTTON_UI_IDENTIFIER,
    ...
    public boolean charTyped(CharacterEvent event) {
        ...
        if(!text.isEmpty()){
            this.elements.searchBarWidget.charTyped(text.charAt(0), 0);
        }
