...
public class MapMarkerWidget extends ModWidget {
    private static final Identifier MAP_MARKERS = Identifier.fromNamespaceAndPath(MiddleEarth.MOD_ID,"textures/gui/widget/map_markers.png");
    private static final Identifier MAP_ARROWS = Identifier.fromNamespaceAndPath(MiddleEarth.MOD_ID,"textures/gui/widget/map_arrows.png");
    ...
    public void draw(GuiGraphicsExtractor context) {
        ...
        if(mouseIsOver){
            ...
            context.setTooltipForNextFrame(client.font, texts, Optional.empty(), drawStart.x + (drawSize.x / 2), drawStart.y + (drawSize.y / 2));
        }

        // draw marker or arrow

        context.blit(RenderPipelines.GUI_TEXTURED, (isArrow) ? MAP_ARROWS : MAP_MARKERS,
