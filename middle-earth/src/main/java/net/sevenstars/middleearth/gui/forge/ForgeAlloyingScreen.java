...
public class ForgeAlloyingScreen extends AbstractContainerScreen<ForgeAlloyingScreenHandler> {
    ...
    public ImageButton leftExtractCycleButton;
    public ImageButton rightExtractCycleButton;
    ...
    protected void init() {
        ...
        this.leftExtractCycleButton = new ImageButton(x + 121, y + 24, 7, 11, LEFT_CYCLE_EXTRACT_BUTTON_TEXTURES, (button) -> {
            if(outputMode == 1){
        ...
        this.rightExtractCycleButton = new ImageButton(x + 162, y + 24, 7, 11, RIGHT_CYCLE_EXTRACT_BUTTON_TEXTURES, (button) -> {
        ...
        this.extractButton = new ImageButton(x + 131, y + 12, 28 ,36, EXTRACT_BUTTON_TEXTURES, (button)-> {
            ...
            ClientPlayNetworking.send(new ForgeOutputPacket(amount, menu.getPos().getX(),menu.getPos().getY(),menu.getPos().getZ(), outputMode));
        ...
        this.modeSwitchToAlloyButton = new ImageButton(x + 23, y + 68, 10 ,10, HEATING_SWITCH_BUTTON_TEXTURES, (button)-> {
            ClientPlayNetworking.send(new ForgeModeSwitchPacket(menu.getPos().getX(),menu.getPos().getY(),menu.getPos().getZ()));
    ...
    public void extractBackground(GuiGraphicsExtractor context, int mouseX, int mouseY, float delta) {
        ...
        context.blit(RenderPipelines.GUI_TEXTURED, TEXTURE, this.leftPos, this.topPos, 0, 0, this.imageWidth, this.imageHeight + 6, TEXTURE_SIZE, TEXTURE_SIZE);

        context.blit(RenderPipelines.GUI_TEXTURED, TEXTURE, this.leftPos + 26, this.topPos + 61, 209, 116, 4, 16, TEXTURE_SIZE, TEXTURE_SIZE);
    ...
    private void renderProgressArrow(GuiGraphicsExtractor context, int x, int y) {
        if(heatingMode) {
            context.blit(RenderPipelines.GUI_TEXTURED, TEXTURE, x + 36, y + 45, 202, 0, COOKING_FIRE_SIZE, COOKING_FIRE_SIZE, TEXTURE_SIZE, TEXTURE_SIZE);
            if(menu.isCooking()) {
                int cookingTime = (int) (menu.getScaledCooking() * COOKING_FIRE_SIZE);
                context.blit(RenderPipelines.GUI_TEXTURED, TEXTURE, x + 36, y + COOKING_FIRE_SIZE + 46 - cookingTime, 202, 15 + COOKING_FIRE_SIZE - cookingTime, COOKING_FIRE_SIZE, cookingTime, TEXTURE_SIZE, TEXTURE_SIZE);
            }
        } else {
            context.blit(RenderPipelines.GUI_TEXTURED, TEXTURE, x + 33, y + 45, 218, 0, 20, 13, TEXTURE_SIZE, TEXTURE_SIZE);
            if(menu.isCooking()) {
                int cookingTime = (int) (menu.getScaledCooking() * 15);
                context.blit(RenderPipelines.GUI_TEXTURED, TEXTURE, x + 33, y + 59 - cookingTime, 218, 15 + 14 - cookingTime, 20, cookingTime, TEXTURE_SIZE, TEXTURE_SIZE);
            }
        }

        if(menu.isCrafting()) {
            context.blit(RenderPipelines.GUI_TEXTURED, TEXTURE, x + 90, y + 16, 212, 83, 7, (int) (menu.getScaledProgress() * PROGRESS_ARROW_SIZE), TEXTURE_SIZE, TEXTURE_SIZE);
        }
    }

    private void renderLiquidStorage(GuiGraphicsExtractor context, int x, int y) {
        int storedLiquid = (int) (menu.getScaledStoredLiquid() * LIQUID_HEIGHT);
        context.blit(RenderPipelines.GUI_TEXTURED, TEXTURE, x + 73, y + 77 - storedLiquid, 211, 76 - storedLiquid, 20, storedLiquid, TEXTURE_SIZE ,TEXTURE_SIZE);
    }

    private void renderLiquidStorageTooltip(GuiGraphicsExtractor context, int mouseX, int mouseY) {
        int x = (width - imageWidth) / 2;
        int y = (height - imageHeight) / 2;
    ...
    protected void extractLabels(GuiGraphicsExtractor context, int mouseX, int mouseY) {
        ...
        context.text(this.font, this.playerInventoryTitle, this.inventoryLabelX, this.inventoryLabelY + 7, -12566464, false);
    }
    ...
    public void extractRenderState(GuiGraphicsExtractor context, int mouseX, int mouseY, float delta) {
        ...
        if(v >= 0) context.blit(RenderPipelines.GUI_TEXTURED, TEXTURE, x + EXTRACT_BUTTON_ITEM_X, y + EXTRACT_BUTTON_ITEM_Y,
            ...
            context.blit(RenderPipelines.GUI_TEXTURED, TEXTURE, x + EXTRACT_BUTTON_ITEM_X + 2, y + EXTRACT_BUTTON_ITEM_Y + 4,
                    204 + u, 30,14, 16, TEXTURE_SIZE ,TEXTURE_SIZE);
            context.blit(RenderPipelines.GUI_TEXTURED, TEXTURE, x + 102, y + 57,
