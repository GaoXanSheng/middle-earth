...
public class RoundShieldModelRenderer implements SpecialModelRenderer<DataComponentMap> {

    private final RoundShieldEntityModel model;
    private final SpriteGetter sprites;

    public RoundShieldModelRenderer(SpriteGetter sprites, RoundShieldEntityModel model) {
        this.model = model;
    ...
    public void submit(@Nullable DataComponentMap data, PoseStack matrices, SubmitNodeCollector collector, int light, int overlay, boolean glint, int seed) {
        ...
        SpriteId sprite = bl ? MEModelLoader.ROUND_SHIELD_BASE : MEModelLoader.ROUND_SHIELD_BASE_NO_PATTERN;
        ...
        if (bl) {
            ...
            collector.submitModelPart(this.model.getPlate(), matrices,
                    ShieldPatternLayers.ROUND_CANVAS.renderType(RenderTypes::bannerPattern),
                    light, overlay, this.sprites.get(ShieldPatternLayers.ROUND_CANVAS),
            ...
            ShieldPatternLayers.submitCanvas(collector, matrices, light, overlay, this.sprites, this.model.getPlate(),
                    ShieldPatternLayers.ROUND_CANVAS, ShieldPatternLayers.round(), canvasColor, bannerPatternsComponent);
        }
        if (glint) {
    ...
    public static record Unbaked() implements SpecialModelRenderer.Unbaked<DataComponentMap> {
        ...
        public MapCodec<RoundShieldModelRenderer.Unbaked> type() {
            return CODEC;
        }

        public SpecialModelRenderer<DataComponentMap> bake(BakingContext context) {
            return new RoundShieldModelRenderer(context.sprites(), new RoundShieldEntityModel(context.entityModelSet().bakeLayer(MiddleEarthClient.ROUND_SHIELD_LAYER)));
        }
