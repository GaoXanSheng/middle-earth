...
public class BannerPatternsME {
    ...
    private static ResourceKey<BannerPattern> key(String id) {
        ...
        return ResourceKey.create(Registries.BANNER_PATTERN, Identifier.fromNamespaceAndPath(MiddleEarth.MOD_ID, id));
    }
    ...
    public static void register(BootstrapContext<BannerPattern> registry, ResourceKey<BannerPattern> key) {
        registry.register(key, new BannerPattern(key.identifier(), "block.%s.banner.".formatted(MiddleEarth.MOD_ID) + key.identifier().toShortLanguageKey()));
    }
}
