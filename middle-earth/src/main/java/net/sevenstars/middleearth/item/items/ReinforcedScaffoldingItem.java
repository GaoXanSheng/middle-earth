        ...
        if (blockState.is(block)) {
            ...
            while (horizontalDistance < ReinforcedScaffoldingBlock.MAX_SUPPORT_DISTANCE) {
                if (!world.isClientSide() && !world.isInWorldBounds(mutable)) {
                    if (context.getPlayer() instanceof ServerPlayer serverPlayer && mutable.getY() > world.getMaxY()) {
                        serverPlayer.sendSystemMessage(
                                Component.translatable("build.tooHigh", world.getMaxY()).withStyle(ChatFormatting.RED),
                                true
                        );
                    }
                    return null;
