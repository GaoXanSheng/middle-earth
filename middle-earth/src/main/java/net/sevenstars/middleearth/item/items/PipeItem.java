...
public class PipeItem extends Item {
    ...
    public InteractionResult use(Level world, Player user, InteractionHand hand) {
        ...
        if (DamageableItemUtils.isFullyDamaged(pipe)) {
            ...
            return refilled ? InteractionResult.CONSUME.heldItemTransformedTo(pipe) : InteractionResult.FAIL;
        }

        startSmoking(pipe, user, world, hand);
        return InteractionResult.CONSUME.heldItemTransformedTo(pipe);
    }

    @Override
    public boolean releaseUsing(ItemStack pipe, Level world, LivingEntity user, int remainingUseTicks) {
    ...
    private void applyCooldown(Player user, ItemStack pipe) {
        user.getCooldowns().addCooldown(pipe, USE_COOLDOWN_TICKS);
    }
