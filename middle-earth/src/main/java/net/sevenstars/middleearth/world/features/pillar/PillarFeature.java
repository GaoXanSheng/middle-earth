...
public class PillarFeature extends Feature<PillarFeatureConfig> {
    ...
    public boolean place(FeaturePlaceContext<PillarFeatureConfig> context) {
        ...
        Optional<Column> optional = Column.scan(structureWorldAccess, blockPos, largeDripstoneFeatureConfig.floorToCeilingSearchRange, SpeleothemUtils::isEmptyOrWater, SpeleothemUtils::isNeitherEmptyNorWater);
        ...
        int j = Mth.clamp(i, largeDripstoneFeatureConfig.columnRadius.minInclusive(), largeDripstoneFeatureConfig.columnRadius.maxInclusive());
        int k = Mth.randomBetweenInclusive(random, largeDripstoneFeatureConfig.columnRadius.minInclusive(), j);
    ...
    protected static boolean canGenerate(LevelAccessor world, BlockPos pos) {
        return world.isStateAtPosition(pos, SpeleothemUtils::isEmptyOrWater);
    }
