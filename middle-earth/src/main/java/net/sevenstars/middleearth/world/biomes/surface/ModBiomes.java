...
public class ModBiomes {
    ...
    public static void createBarrowDownsBiome(BootstrapContext<Biome> context, ResourceKey<Biome> biomeRegistryKey) {
        ...
        vegetation.add(VegetationPlacements.PALE_MOSS_PATCH);

        registerBiome(context, biomeRegistryKey, spawnSettings, generationSettings, new AmbientParticle(ModParticleTypes.BIOME_FOG_PARTICLE, 0.003f), 0.5f, true);
    }
    ...
    public static void createGreyPlainsBiome(BootstrapContext<Biome> context, ResourceKey<Biome> biomeRegistryKey, int step) {
        ...
        if(step == 0) {
            ModBiomeFeatures.addMixedWildWheatPatch(vegetation);
            ModSpawnSettingsBuilder.addFarmAnimals(spawnSettings);
            vegetation.add(VegetationPlacements.PATCH_DRY_GRASS_DESERT);
    ...
    public static void createLindonBiome(BootstrapContext<Biome> context, ResourceKey<Biome> biomeRegistryKey, int step) {
        ...
        if(step == 0) { // Plains
            ...
            ModBiomeFeatures.addMegaBirchTrees(vegetation);
            ModBiomeFeatures.addRareMegaOakTrees(vegetation);
            vegetation.add(VegetationPlacements.PATCH_LEAF_LITTER);
            ...
            ModBiomeFeatures.addWhiteFlowers(vegetation);
            vegetation.add(VegetationPlacements.PATCH_LEAF_LITTER);
            vegetation.add(TreePlacements.FALLEN_BIRCH_TREE);
    ...
    public static void registerBiome(BootstrapContext<Biome> context, ResourceKey<Biome> biomeRegistryKey, MobSpawnSettings.Builder spawnSettings, BiomeGenerationSettings.Builder generationSettings, float temperature, boolean precipitation, boolean... removeDefaultOres) {
        ...
        surfaceStructures = surfaceStructures.stream().sorted(Comparator.comparing(a -> a.identifier().toString())).toList();
        vegetation = vegetation.stream().sorted(Comparator.comparing(a -> a.identifier().toString())).toList();
        for(int i = 0; i < vegetation.size() - 1; i++) {
            if(vegetation.get(i).identifier().toString().equals(vegetation.get(i + 1).identifier().toString())) {
                throw new IllegalStateException("Duplicate value in list for: " + vegetation.get(i).identifier().toString());
            }
        }
        ...
        for (ResourceKey<PlacedFeature> feature: undergroundOres.stream().sorted(Comparator.comparing(a -> a.identifier().toString())).toList()) {
        ...
        Biome biome = (new Biome.BiomeBuilder())
                ...
                .downfall(0.5F)
                .setAttribute(EnvironmentAttributes.SKY_COLOR, biomeColorsDTO.skyColor)
                .setAttribute(EnvironmentAttributes.FOG_COLOR, biomeColorsDTO.fogColor)
                .setAttribute(EnvironmentAttributes.WATER_FOG_COLOR, biomeColorsDTO.waterFogColor)
    ...
    public static void registerBiome(BootstrapContext<Biome> context, ResourceKey<Biome> biomeRegistryKey, MobSpawnSettings.Builder spawnSettings, BiomeGenerationSettings.Builder generationSettings, AmbientParticle particleConfig, float temperature, boolean precipitation, boolean... removeDefaultOres) {
        if(removeDefaultOres.length == 0) {
            undergroundOres.add(OrePlacements.ORE_DIRT);
            undergroundOres.add(OrePlacements.ORE_GRAVEL);
            undergroundOres.add(OrePlacements.ORE_GRANITE_UPPER);
        ...
        Biome biome = (new Biome.BiomeBuilder())
                ...
                .setAttribute(EnvironmentAttributes.FOG_COLOR, biomeColorsDTO.fogColor)
                .setAttribute(EnvironmentAttributes.WATER_FOG_COLOR, biomeColorsDTO.waterFogColor)
                .setAttribute(EnvironmentAttributes.AMBIENT_PARTICLES, AmbientParticle.of(particleConfig.particle(), particleConfig.probability()))
                .specialEffects((new BiomeSpecialEffects.Builder())
                        .waterColor(biomeColorsDTO.waterColor)
                        .grassColorOverride(biomeColorsDTO.grassColor)
                        .foliageColorOverride(biomeColorsDTO.foliageColor)
