...
public class SpruceTrunkPlacer extends TrunkPlacer {
    ...
    protected List<FoliagePlacer.FoliageAttachment> createTrunk(WorldGenLevel world, BiConsumer<BlockPos, BlockState> replacer, RandomSource random, BlockPos.MutableBlockPos mutable,
                ...
                for (int j = 1; j < 2; j++) {
                    ...
                    replacer.accept(startPos.above(i).relative(direction, j),
                            (BlockState) Function.identity().apply(config.trunkProvider
                                    .getState(world, random, startPos.above(i).relative(direction, j))
