...
public class RaceStatTooltip {
    ...
    public static void draw(Race race, LivingEntity entity, GuiGraphicsExtractor context, Font renderer, int x, int y, List<AttributePoolElement> playerAttributes, boolean detailed){
        tooltipText = new ArrayList<>();
        addRaceName(race);
        addAttributeListHeader();

        boolean hasAttribute = false;
        var registry = entity.level().registryAccess().lookup(Registries.ATTRIBUTE).get();
        ...
        context.setComponentTooltipForNextFrame(renderer, tooltipText, x, y);
    }
    ...
    private static boolean addAttributeLine(LivingEntity entity, Registry<Attribute> registry, AttributePoolElement currentEntityAttribute, List<AttributePoolElement> nextBoundAttributes, boolean detailed) {
        ...
        boolean buffIsReversed = registry.get(attributeId).get().is(TagKey.create(Registries.ATTRIBUTE, MiddleEarth.of("is_buff_reversed")));
