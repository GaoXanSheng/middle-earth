...
public class FactionLookup {
    public static List<Faction> getAllFactions(Level world) {
        return world.registryAccess().lookupOrThrow(DynamicRegistriesME.FACTION).stream().toList();
    }
    public static Faction getFactionById(Level world, Identifier id) throws FactionIdentifierException {
        if(id == null)
            return null;
        Faction faction = world.registryAccess().lookupOrThrow(DynamicRegistriesME.FACTION).getValue(id);
        if(faction == null)
            throw new FactionIdentifierException();
        return faction;
    }

    public static HashMap<Identifier, Faction> getFactionsByDisposition(Level world, DispositionType dispositionType){
