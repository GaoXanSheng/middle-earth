...
public class SpiderPounceTask<E extends Mob> extends Behavior<E> {
	...
	@Override
	protected boolean checkExtraStartConditions(ServerLevel serverWorld, E mobEntity) {
	...
	@Override
	protected boolean canStillUse(ServerLevel serverWorld, E mobEntity, long l) {
	...
	protected Optional<Target> removeRandomTarget(ServerLevel world) {
		Optional<Target> optional = WeightedRandom.getRandomItem(world.getRandom(), this.potentialTargets, Target::weight);
	...
	public record Target(Vec3 pos, int weight) {
	}
}
