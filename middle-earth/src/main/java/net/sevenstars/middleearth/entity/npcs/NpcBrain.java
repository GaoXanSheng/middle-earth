...
public class NpcBrain {
    ...
    public static Brain.Provider<NpcEntity> brainProvider() {
        return Brain.provider(MEMORY_MODULES, SENSORS, NpcBrain::getActivities);
    }

    public static java.util.List<ActivityData<NpcEntity>> getActivities(NpcEntity npcEntity) {
    ...
    protected static Brain<NpcEntity> create(NpcEntity npcEntity, Brain.Packed packed) {
        Brain.Provider<NpcEntity> profile = brainProvider();
        Brain<NpcEntity> brain = profile.makeBrain(npcEntity, packed);
    ...
    public static void setRangedActivities(Brain<NpcEntity> brain, NpcEntity npcEntity, RangedCombatArchetypeRuntimeData runtimeData){
        ...
        brain.addActivity(Activity.FIGHT, ImmutableList.of(
                        ...
                        Pair.of(1, StopAttackingIfTargetInvalid.create(((world, target) -> shouldForgetTarget(world, target, npcEntity)))),
    ...
    public static void updateActivities(NpcEntity npc) {
        ...
        npc.getBrain().updateActivityFromSchedule(npc.level().environmentAttributes(), npc.level().getGameTime(), npc.position());
    }

    private static ActivityData<NpcEntity> initCoreActivity() {
    ...
    private static void addCoreActivities(Brain<NpcEntity> brain) {
        ...
        brain.addActivity(Activity.CORE, ActivityData.createPriorityPairs(0, ImmutableList.of(
                new MoveToTargetSink(),
                new LookAtTargetSink(45, 90),
    ...
    private static ActivityData<NpcEntity> initIdleActivity() {
    ...
    private static void addFightActivities(NpcEntity npc, Brain<NpcEntity> brain) {
        ...
        brain.addActivity(Activity.FIGHT, ImmutableList.of(
                        Pair.of(0, StartAttacking.create(NpcBrain::getAttackTarget)),
                        Pair.of(1, StopAttackingIfTargetInvalid.create(((world, target) -> shouldForgetTarget(world, target, npc)))),
                        Pair.of(2, new NpcMountedApproachTask()),
    ...
    private static Optional<? extends LivingEntity> getAttackTarget(ServerLevel serverWorld, NpcEntity npc) {
        return npc.getBrain().getMemory(MemoryModuleType.NEAREST_ATTACKABLE);
    }

    private static boolean shouldForgetTarget(ServerLevel world, LivingEntity target, NpcEntity npc) {
