...
public class BroadhoofGoatEntity extends AbstractBeastEntity {
    ...
    protected void readAdditionalSaveData(ValueInput view) {
        ...
        this.entityData.set(BEADS, view.getIntOr("Beads", 0));
        this.entityData.set(HAIR, view.getBooleanOr("Hair", false));
        this.entityData.set(LEFT_HORN, view.getBooleanOr("HasLeftHorn", true));
        this.entityData.set(RIGHT_HORN, view.getBooleanOr("HasRightHorn", true));
        this.entityData.set(BRUSHED_BEARD, view.getBooleanOr("HasBrushedBeard", false));
    ...
    public AgeableMob getBreedOffspring(ServerLevel world, AgeableMob entity) {
        ...
        BroadhoofGoatEntity broadhoofEntity2 = EntitiesME.BROADHOOF_GOAT.create(world, EntitySpawnReason.BREEDING);
    ...
    public boolean doHurtTarget(ServerLevel world, Entity target) {
        if(!world.isClientSide() && super.doHurtTarget(world, target)) {
    ...
    public void chargeAttack() {
        ...
        for(Entity entity : entities) {
            if(entity != this.getOwner() && entity != this && !this.getPassengers().contains(entity) && !this.level().isClientSide()) {
                entity.hurtServer((ServerLevel) this.level(), entity.damageSources().mobAttack(this), getAttackDamage());
    ...
    public ItemStack getGoatHornStack() {
        ...
        return (ItemStack)this.level().registryAccess().lookupOrThrow(Registries.INSTRUMENT).getRandomElementOf(tagKey, random).map((registryEntry) -> {
    ...
    public <T> T get(DataComponentType<? extends T> type) {
        return type == DataComponentTypesME.GOAT_VARIANT ? castComponentValue((DataComponentType<T>)type, this.getGoatColor()) : super.get(type);
    }

    @Override
    protected void applyImplicitComponents(DataComponentGetter from) {
        this.applyImplicitComponentIfPresent(from, DataComponentTypesME.GOAT_VARIANT);
