...
public class EntitiesME {
    ...
    public static final EntityType<StoneTrollEntity> STONE_TROLL = register("stone_troll",
            EntityType.Builder.of(StoneTrollEntity::new, MobCategory.CREATURE).sized(1.4f, 3.4f));
    ...
    public static <T extends Entity> EntityType<T> registerEntity(String name, EntityType.EntityFactory<T> entity, MobCategory spawnGroup,
        ...
        return Registry.register(BuiltInRegistries.ENTITY_TYPE, MiddleEarth.of(name), EntityType.Builder.of(entity, spawnGroup).sized(width, height).build(keyOf("name")));
    }
    ...
    private static <T extends Entity> EntityType<T> register(ResourceKey<EntityType<?>> key, EntityType.Builder<T> type) {
        ...
        RegistryAliasesME.aliases.add(new RegistryAliasesME.Alias(BuiltInRegistries.ENTITY_TYPE, entityType.toShortString()));
        return entityType;
    }
    ...
    private static ResourceKey<EntityType<?>> keyOf(String id) {
        return ResourceKey.create(Registries.ENTITY_TYPE, Identifier.fromNamespaceAndPath(MiddleEarth.MOD_ID, id));
    }
    ...
    private static void registerSpawnRestrictions(){
        SpawnPlacementType onGroundLocation = SpawnPlacementTypes.ON_GROUND;
        Heightmap.Types heightmapType = Heightmap.Types.MOTION_BLOCKING_NO_LEAVES;

        SpawnPlacements.register(SHELOBITE_LARVA, onGroundLocation, heightmapType, ShelobiteLarvaEntity::canSpawn);
        SpawnPlacements.register(SHELOBITE_SCUTTLER, onGroundLocation, heightmapType, ShelobiteScuttlerEntity::canSpawn);
        SpawnPlacements.register(SPAWN_OF_SHELOB, onGroundLocation, heightmapType, SpawnOfShelobEntity::canSpawn);

        SpawnPlacements.register(CAVE_TROLL, onGroundLocation, heightmapType, CaveTrollEntity::canSpawn);
        SpawnPlacements.register(NPC, onGroundLocation, heightmapType, NpcEntity::canSpawn);
    }
    ...
    static {
        ...
        }), MobCategory.MISC).noLootTable().sized(1.2F, 1.2F).eyeHeight(0.625F).clientTrackingRange(10));
