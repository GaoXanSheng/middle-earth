...
public class GreatHornEntity extends AbstractBeastEntity implements Evader {
    ...
    protected void defineSynchedData(SynchedEntityData.Builder builder) {
        ...
        Holder<GreatHornVariant> greatHornVariantRegistryEntry = VariantUtils.getDefaultOrAny(this.registryAccess(), GreatHornVariantRegistry.DEFAULT);
    ...
    protected void addAdditionalSaveData(ValueOutput view) {
        ...
        VariantUtils.writeVariant(view, this.getRegistryVariant());
    }

    @Override
    protected void readAdditionalSaveData(ValueInput view) {
        super.readAdditionalSaveData(view);
        VariantUtils.readVariant(view, DynamicRegistriesME.GREAT_HORN_VARIANTS).ifPresent(this::setVariant);
    ...
    public InteractionResult mobInteract(Player player, InteractionHand hand) {
        ...
        if(itemStack.is(Items.DYE.blue()) && !this.entityData.get(BLUE_SADDLE)) {
            this.entityData.set(BLUE_SADDLE, true);
        } else if(itemStack.is(Items.DYE.red()) && this.entityData.get(BLUE_SADDLE)) {
    ...
    public AgeableMob getBreedOffspring(ServerLevel world, AgeableMob entity) {
        GreatHornEntity greatHornEntity2 = EntitiesME.GREAT_HORN.create(world, EntitySpawnReason.BREEDING);
    ...
    public void handleStartJump(int height) {
        if(this.hasControllingPassenger()) {
            ...
            if(!this.getControllingPassenger().isSprinting()) {
                ...
                List<Entity> entities = this.level().getEntities(this, this.getBoundingBox().inflate(2.5,2,2.5));

                for(Entity entity : entities) {
                    if(!this.getPassengers().contains(entity)) {
                        if(!this.level().isClientSide()) {
                            entity.hurtServer((ServerLevel) this.level(), entity.damageSources().mobAttack(this), jumpPercentage * getAttackDamage());
                        }
                        double dx = entity.getX() - this.getX();
                        double dz = entity.getZ() - this.getZ();

                        Vec3 velocity = new Vec3(dx, 1.25f + getRandom().nextFloat() * 0.5f, dz).normalize();
                        velocity.scale(jumpPercentage);
    ...
    public void tick() {
        ...
        if (this.level().isClientSide() && bowAnimationState.isStarted()) {
            if(random.nextInt(2) == 0) {
                ...
                this.level().addParticle(SpellParticleOption.create(ParticleTypes.INSTANT_EFFECT, 0.0f, 0.75f + this.random.nextFloat(), 0.0f, 0.0f),
                        this.getX() + randPos.x, this.getY() + randPos.y, this.getZ() + randPos.z, 0.0, 0.0, 0.0);
            }
        }
    }
    ...
    public SpawnGroupData finalizeSpawn(ServerLevelAccessor world, DifficultyInstance difficulty, EntitySpawnReason spawnReason,
        ...
        if (entityData instanceof GreatHornData greatHornData) {
            ...
            Optional<? extends Holder<GreatHornVariant>> optional = VariantUtils.selectVariantToSpawn(SpawnContext.create(world, this.blockPosition()), DynamicRegistriesME.GREAT_HORN_VARIANTS);
    ...
    public static boolean canSpawn(EntityType<GreatHornEntity> type, ServerLevelAccessor serverWorldAccess, EntitySpawnReason spawnReason, BlockPos blockPos, RandomSource random) {
