...
public class FeetModel extends EntityModel<NpcEntityRenderState> {
    ...
    public void setupAnim(NpcEntityRenderState state) {
        ...
        float h = state.walkAnimationSpeed;

        this.rightLeg.xRot = Mth.cos(g * 0.6662F) * 1.4F * h / state.speedValue;
        this.leftLeg.xRot = Mth.cos(g * 0.6662F + (float)Math.PI) * 1.4F * h / state.speedValue;
