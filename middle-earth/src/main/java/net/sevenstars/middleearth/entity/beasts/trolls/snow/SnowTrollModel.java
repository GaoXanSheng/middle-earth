...
public class SnowTrollModel extends EntityModel<TrollEntityRenderState> {
    ...
    private final KeyframeAnimation attackinggAnimation;
    ...
    public SnowTrollModel(ModelPart root) {
        ...
        this.walkingAnimation = SnowTrollAnimations.WALKING.bake(root);
        this.attackinggAnimation = SnowTrollAnimations.ATTACK.bake(root);
        this.chargingAnimation = SnowTrollAnimations.CHARGING.bake(root);
        this.throwingAnimation = SnowTrollAnimations.THROWING.bake(root);
    }
    ...
    @Override
    public void setupAnim(TrollEntityRenderState state) {
        super.setupAnim(state);
        this.setHeadAngles(state.yRot, state.xRot);

        this.walkingAnimation.applyWalk(state.walkAnimationPos, state.walkAnimationSpeed, 1.0F, 2.5F);
        this.attackinggAnimation.apply(state.attackAnimationState, state.ageInTicks);
