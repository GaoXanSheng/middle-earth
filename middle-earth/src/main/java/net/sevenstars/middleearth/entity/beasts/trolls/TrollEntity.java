...
public class TrollEntity extends AbstractBeastEntity {
    ...
    protected void readAdditionalSaveData(ValueInput view) {
        ...
        this.setHasChest(view.getBooleanOr("ChestedTroll", false));
        this.createInventory();
        if (this.hasChest()) {
            Iterator list = view.listOrEmpty("Items", ItemStackWithSlot.CODEC).iterator();

            while(list.hasNext()) {
                ItemStackWithSlot stackWithSlot = (ItemStackWithSlot)list.next();
        ...
        // Saddles persist through the vanilla EntityEquipment system; Mob.isSaddled() derives from it.
    }
    ...
    public boolean doHurtTarget(ServerLevel world, Entity target) {
        ...
        boolean bl = target.hurtServer(world, this.damageSources().mobAttack(this), g);
        if (bl) {
            double d;
    ...
    public void chargeAttack() {
        ...
        for(Entity entity : entities) {
            if(entity != this.getOwner() && entity != this && !this.getPassengers().contains(entity)) {
                if(level() instanceof ServerLevel serverWorld)
                    entity.hurtServer(serverWorld, entity.damageSources().mobAttack(this), 16.0f);
            }
        }
