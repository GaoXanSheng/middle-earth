...
public class SpawnOfShelobModel extends EntityModel<SpawnOfShelobRenderState> {
    ...
    private final KeyframeAnimation walkingBlockAnimation;
    private final KeyframeAnimation pounceAnimation;

    public SpawnOfShelobModel(ModelPart root) {
        super(root);
        this.root = root.getChild("root");

        this.idleAnimation = SpawnOfShelobAnimations.SPAWN_OF_SHELOB_IDLE.bake(root);
        this.walkingAnimation = SpawnOfShelobAnimations.SPAWN_OF_SHELOB_WALK.bake(root);
        this.biteAnimation = SpawnOfShelobAnimations.SPAWN_OF_SHELOB_BITE.bake(root);
        this.walkingBlockAnimation = SpawnOfShelobAnimations.SPAWN_OF_SHELOB_BLOCK.bake(root);
        this.pounceAnimation = SpawnOfShelobAnimations.SPAWN_OF_SHELOB_POUNCE.bake(root);
    }
    ...
    @Override
    public void setupAnim(SpawnOfShelobRenderState state) {
        ...
        if(state.climbingTicks > 0 && climbingPercentage > leapingPercentage) {
            ...
            this.walkingAnimation.applyWalk(state.ageInTicks, 0.4f, 1.75F, 2F);
            return;
        } else if(state.leapingTicks > 0 && leapingPercentage > climbingPercentage) {
            this.root.xRot = -0.8f * leapingPercentage;
            this.walkingAnimation.applyWalk((float)state.leapingTicks / 3.1f, 0.75f, 2.2F, 2.5F);
            return;
        }

        if(state.walkAnimationSpeed <= 0.4) {
            this.idleAnimation.apply(state.idleAnimationState, state.ageInTicks, 0.75f);
        } else if(state.blockAnimationState.isStarted()) {
            this.walkingBlockAnimation.applyWalk(state.walkAnimationPos, state.walkAnimationSpeed, 2.2F, 2.25F);
        } else {
            this.walkingAnimation.applyWalk(state.walkAnimationPos, state.walkAnimationSpeed, 2.25F, 2.5F);
        }

        if(state.biteAnimationState.isStarted()) {
            this.biteAnimation.apply(state.biteAnimationState, state.ageInTicks, 1.3f);
        }
        if(state.pounceAnimationState.isStarted()) {
            this.pounceAnimation.apply(state.pounceAnimationState, state.ageInTicks, 1.0f);
        }
    }
}