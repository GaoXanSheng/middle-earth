...
public class CaveTrollEntityModel extends EntityModel<CaveTrollEntityRenderState> {
    ...
    private final KeyframeAnimation scavengingAnimation;
    private final KeyframeAnimation startSittingAnimation;
    private final KeyframeAnimation stopSittingAnimation;
    private final KeyframeAnimation startSleepingAnimation;
    private final KeyframeAnimation sleepingAnimation;
    private final KeyframeAnimation stopSleepingAnimation;
    private final KeyframeAnimation roaringAnimation;
    private final KeyframeAnimation smashingAnimation;
    ...
    protected CaveTrollEntityModel(ModelPart root) {
        ...
        walkingAnimation = CaveTrollAnimations.PASSIVE_WALK.bake(root);
        chaseAnimation = CaveTrollAnimations.RUN.bake(root);
        scavengingAnimation = CaveTrollAnimations.INSPECT.bake(root);
        startSittingAnimation = CaveTrollAnimations.STANDING_TO_SITTING.bake(root);
        stopSittingAnimation = CaveTrollAnimations.STANDING_FROM_SITTING.bake(root);
        startSleepingAnimation = CaveTrollAnimations.SLEEP_LAYING_DOWN.bake(root);
        sleepingAnimation = CaveTrollAnimations.SLEEP_LAYING_DOWN.bake(root);
        stopSleepingAnimation = CaveTrollAnimations.SLEEP_LAYING_DOWN_STANDING_UP.bake(root);
        roaringAnimation = CaveTrollAnimations.SCREAM.bake(root);
        smashingAnimation = CaveTrollAnimations.OVERHAND_SMASH.bake(root);
    }
    ...
    @Override
    public void setupAnim(CaveTrollEntityRenderState state) {
        ...
        if(!state.isSprinting && !(state.conrollingPassenger != null && state.conrollingPassenger.isSprinting()) && !state.isCharging) {
            this.walkingAnimation.applyWalk(state.walkAnimationPos, state.walkAnimationSpeed, 10.0f, 10.0f);
        }
        else {
            this.chaseAnimation.applyWalk(state.walkAnimationPos, state.walkAnimationSpeed, 2.0f, 2.0f);
        }

        this.scavengingAnimation.apply(state.scavengingAnimationState, state.ageInTicks);
        this.startSittingAnimation.apply(state.startSittingAnimationState, state.ageInTicks);
        this.stopSittingAnimation.apply(state.stopSittingAnimationState, state.ageInTicks);
        this.startSleepingAnimation.apply(state.startSleepingAnimationState, state.ageInTicks);
        this.sleepingAnimation.apply(state.sleepingAnimationState, state.ageInTicks);
        this.stopSleepingAnimation.apply(state.stopSleepingAnimationState, state.ageInTicks);
        this.roaringAnimation.apply(state.roaringAnimationState, state.ageInTicks);
        this.smashingAnimation.apply(state.smashingAnimationState, state.ageInTicks);
    }
