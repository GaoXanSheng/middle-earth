...
public class BroadhoofGoatModel extends EntityModel<BroadhoofGoatEntityRenderState> {
    ...
    private final KeyframeAnimation eatingAnimation;
    private final KeyframeAnimation rammingAnimation;
    private final KeyframeAnimation layingAnimation;
    private final KeyframeAnimation standingAnimation;
    private final KeyframeAnimation lyingAnimation;
    private final KeyframeAnimation chargingAnimation;
    private final KeyframeAnimation jumpingAnimation;
    ...
    public BroadhoofGoatModel(ModelPart root) {
        ...
        this.walkingAnimation = BroadhoofGoatAnimations.WALK.bake(root);
        this.runningAnimation = BroadhoofGoatAnimations.RUN.bake(root);
        this.eatingAnimation = BroadhoofGoatAnimations.EAT.bake(root);
        this.rammingAnimation = BroadhoofGoatAnimations.RAM_ATTACK.bake(root);
        this.layingAnimation = BroadhoofGoatAnimations.LAY_DOWN.bake(root);
        this.standingAnimation = BroadhoofGoatAnimations.STAND_UP.bake(root);
        this.lyingAnimation = BroadhoofGoatAnimations.LYING.bake(root);
        this.chargingAnimation = BroadhoofGoatAnimations.CHARGE_ATTACK.bake(root);
        this.jumpingAnimation = BroadhoofGoatAnimations.JUMP.bake(root);
    }
    ...
    @Override
    public void setupAnim(BroadhoofGoatEntityRenderState state) {
        super.setupAnim(state);

        for(int i = 0 ; i < BroadhoofGoatHorns.values().length; i++) {
            this.leftHorns[i].visible = (state.horns.getId() == i) && state.hasLeftHorn && !state.isBaby;
            this.rightHorns[i].visible = (state.horns.getId() == i) && state.hasRightHorn && !state.isBaby;
        }
        ...
        if(!state.isSprinting && !(state.conrollingPassenger != null && state.conrollingPassenger.isSprinting()) && !state.isCharging) {
            this.walkingAnimation.applyWalk(state.walkAnimationPos, state.walkAnimationSpeed, 4f, 4f);
        }
        else {
            this.runningAnimation.applyWalk(state.walkAnimationPos, state.walkAnimationSpeed, 1.2f, 1.2f);
        }

        this.eatingAnimation.apply(state.idleAnimationState, state.ageInTicks);
        this.rammingAnimation.apply(state.attackAnimationState, state.ageInTicks);
        this.layingAnimation.apply(state.startSittingAnimationState, state.ageInTicks);
        this.standingAnimation.apply(state.stopSittingAnimationState, state.ageInTicks);
        this.lyingAnimation.apply(state.sittingAnimationState, state.ageInTicks);
        this.chargingAnimation.apply(state.chargeAnimationState, state.ageInTicks);
        this.jumpingAnimation.apply(state.jumpAnimationState, state.ageInTicks);
