...
public class WargModel extends EntityModel<WargEntityRenderState> {
    ...
    public WargModel(ModelPart root) {
        ...
        this.walkingAnimation = WargAnimations.WALK.bake(root);
        this.runningAnimation = WargAnimations.RUN.bake(root);
        this.idleAnimation = WargAnimations.IDLE.bake(root);
        this.bitingAnimation = WargAnimations.BITE.bake(root);
        this.startSittingAnimation = WargAnimations.STANDING_TO_SITTING.bake(root);
        this.stopSittingAnimation = WargAnimations.SITTING_TO_STANDING.bake(root);
        this.sittingAnimation = WargAnimations.SITTING.bake(root);
    }
    ...
    @Override
    public void setupAnim(WargEntityRenderState state) {
        ...
        if(!running) {
            ...
            this.walkingAnimation.applyWalk(state.walkAnimationPos, state.walkAnimationSpeed, speed, 2f);
        }
        else {
            this.runningAnimation.applyWalk(state.walkAnimationPos, state.walkAnimationSpeed, 1.4f, 1.2f);
        }

        this.idleAnimation.apply(state.idleAnimationState, state.ageInTicks);
        this.bitingAnimation.apply(state.attackAnimationState, state.ageInTicks);
