...
public class CaveTrollRoarTask extends Behavior<CaveTrollEntity> {
    ...
    protected void start(ServerLevel world, CaveTrollEntity troll, long time) {
        ...
        List<Entity> entities = world.getEntities(troll, troll.getBoundingBox().inflate(15, 15, 15)); // Get nearby entities
        for(Entity entity : entities) { // Run through all entities
            if(troll.isValidTarget(entity)) { // Apply bad effects if entity is valid target
                ((LivingEntity)entity).addEffect(new MobEffectInstance(MobEffects.WEAKNESS, 200), troll);
                ((LivingEntity)entity).addEffect(new MobEffectInstance(MobEffects.SLOWNESS, 100), troll);
            }
        }
    }
