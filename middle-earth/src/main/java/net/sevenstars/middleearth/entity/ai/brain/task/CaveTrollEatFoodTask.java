...
public class CaveTrollEatFoodTask extends Behavior<CaveTrollEntity> {
    ...
    public static TagKey<Item> TROLL_FOOD = TagKey.create(Registries.ITEM, Identifier.fromNamespaceAndPath(MiddleEarth.MOD_ID, "troll_food"));
    ...
    protected void tick(ServerLevel world, CaveTrollEntity entity, long time) {
        if((time - startTime) > 60 && !entity.getMainHandItem().isEmpty()) {
            ItemParticleOption particles = new ItemParticleOption(ParticleTypes.ITEM, entity.getMainHandItem().getItem());
