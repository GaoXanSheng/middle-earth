...
public class DeerEntityModel extends EntityModel<DeerEntityRenderState> {
    ...
    protected DeerEntityModel(ModelPart root) {
        super(root);
        this.walkingAnimation = DeerEntityAnimations.WALK.bake(root);
        this.runningAnimation = DeerEntityAnimations.RUN.bake(root);
    }
    ...
    @Override
    public void setupAnim(DeerEntityRenderState state) {
        super.setupAnim(state);

        if(state.isRunning) {
            this.runningAnimation.applyWalk(state.walkAnimationPos, state.walkAnimationSpeed, 1.5F, 1.0F);
        }
        else {
