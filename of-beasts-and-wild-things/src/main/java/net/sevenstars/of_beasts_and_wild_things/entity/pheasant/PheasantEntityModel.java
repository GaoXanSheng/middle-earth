...
public class PheasantEntityModel extends EntityModel<PheasantEntityRenderState> {
    ...
    public PheasantEntityModel(ModelPart root) {
        super(root);
        this.walkingAnimation = PheasantEntityAnimations.WALK.bake(root);
        this.idlingAnimation = PheasantEntityAnimations.IDLE.bake(root);
        this.diggingAnimation = PheasantEntityAnimations.DIGGING.bake(root);
        this.flapAnimation = PheasantEntityAnimations.FLAP.bake(root);
    }
    ...
    @Override
    public void setupAnim(PheasantEntityRenderState state) {
        super.setupAnim(state);

        this.walkingAnimation.applyWalk(state.walkAnimationPos, state.walkAnimationSpeed, 5.0F, 5.0F);
        this.idlingAnimation.apply(state.idleAnimationState, state.ageInTicks);
        this.diggingAnimation.apply(state.diggingAnimationState, state.ageInTicks, 1.2F);
        this.flapAnimation.apply(state.flapAnimationState, state.ageInTicks, 2.0F);
    }
}