...
public class EatCropTask extends Behavior<SnailEntity> {
    ...
    protected boolean checkExtraStartConditions(ServerLevel serverWorld, SnailEntity snailEntity) {
        return serverWorld.getBlockState(snailEntity.blockPosition().above()).is(BlockTags.CROPS) && serverWorld.getGameRules().get(GameRules.MOB_GRIEFING);
    }
    ...
    @Override
    protected void start(ServerLevel world, SnailEntity entity, long time) {
        ...
        // 5% chance of spawning a baby snail
            ...
            if(babySnail != null) {
                babySnail.absSnapTo(entity.getX(), entity.getY(), entity.getZ());
