...
public class SwanBrain {
    private static final ImmutableList<SensorType<? extends Sensor<? super SwanEntity>>> SENSORS = ImmutableList.of(
            SensorType.HURT_BY,
    ...
    public static final Brain.Provider<SwanEntity> BRAIN_PROVIDER = Brain.provider(MEMORY_MODULES, SENSORS, SwanBrain::createActivities);

    public static Brain<SwanEntity> create(SwanEntity swanEntity, Brain.Packed packed) {
        Brain<SwanEntity> brain = BRAIN_PROVIDER.makeBrain(swanEntity, packed);
    ...
    private static List<ActivityData<SwanEntity>> createActivities(SwanEntity swanEntity) {
        return ImmutableList.of(
                coreActivity(),
                idleActivity(),
                restActivity(),
                fightActivity(swanEntity),
                babyIdleActivity(),
                babyRestActivity()
        );
    }

    private static ActivityData<SwanEntity> coreActivity() {
        return ActivityData.create(Activity.CORE, 0, ImmutableList.of(
                new MoveToTargetSink(),
                new LookAtTargetSink(45, 90),
                DefendHomeTask.create(5),
                StartAttacking.create((world, swan) -> swan.getHurtBy()),
                new CountDownCooldownTicks(MemoryModulesWT.EGG_COOLDOWN)
        ));
    }

    private static ActivityData<SwanEntity> idleActivity() {
        return ActivityData.create(Activity.IDLE, ImmutableList.of(
                Pair.of(0, new AnimalMakeLove(EntitiesWT.SWAN)),
                Pair.of(1, new RunOne<SwanEntity>(ImmutableMap.of(MemoryModuleType.HOME, MemoryStatus.VALUE_ABSENT), ImmutableList.of(
                        Pair.of(SearchForHomeTask.create(BlocksWT.BIRD_NEST), 2),
                        Pair.of(RandomStroll.stroll(1.0F), 1)
                ))),
                Pair.of(2, new RunOne<SwanEntity>(ImmutableMap.of(MemoryModuleType.HOME, MemoryStatus.VALUE_PRESENT), ImmutableList.of(
    ...
    private static ActivityData<SwanEntity> restActivity() {
        return ActivityData.create(Activity.REST, ImmutableList.of(
                Pair.of(0, new RunOne<SwanEntity>(ImmutableMap.of(MemoryModuleType.HOME, MemoryStatus.VALUE_ABSENT), ImmutableList.of(
                        Pair.of(SearchForHomeTask.create(BlocksWT.BIRD_NEST), 2),
                        Pair.of(RandomStroll.stroll(1.0F), 1)
                ))),
    ...
    private static ActivityData<SwanEntity> fightActivity(SwanEntity swan) {
        return ActivityData.create(Activity.FIGHT, ImmutableList.of(
                Pair.of(0, StartAttacking.create(SwanBrain::getAttackTarget)),
                Pair.of(1, StopAttackingIfTargetInvalid.create(((world, target) -> shouldForgetTarget(world, target, swan)))),
    ...
    private static ActivityData<SwanEntity> babyIdleActivity() {
        return ActivityData.create(ActivitiesAPI.BABY_IDLE, ImmutableList.of(
                Pair.of(0, new RunOne<SwanEntity>(ImmutableMap.of(MemoryModuleType.HOME, MemoryStatus.VALUE_ABSENT), ImmutableList.of(
                        Pair.of(SearchForHomeTask.create(BlocksWT.BIRD_NEST), 2),
                        Pair.of(RandomStroll.stroll(1.0F), 1)
                ))),
                Pair.of(1, new RunOne<SwanEntity>(ImmutableMap.of(MemoryModuleType.NEAREST_VISIBLE_ADULT, MemoryStatus.VALUE_ABSENT), ImmutableList.of(
                        Pair.of(RandomStroll.stroll(1.0F), 1),
    ...
    private static ActivityData<SwanEntity> babyRestActivity() {
        return ActivityData.create(ActivitiesAPI.BABY_REST, ImmutableList.of(
                Pair.of(0, new RunOne<SwanEntity>(ImmutableMap.of(MemoryModuleType.HOME, MemoryStatus.VALUE_ABSENT), ImmutableList.of(
                        Pair.of(SearchForHomeTask.create(BlocksWT.BIRD_NEST), 2),
                        Pair.of(RandomStroll.stroll(1.0F), 1)
                ))),
    ...
    public static void updateActivities(SwanEntity swan) {
        ...
        swan.getBrain().updateActivityFromSchedule(swan.level().environmentAttributes(), swan.level().getGameTime(), swan.position());
    }

    private static boolean shouldForgetTarget(ServerLevel world, LivingEntity target, SwanEntity swan) {
    ...
    private static Optional<? extends LivingEntity> getAttackTarget(ServerLevel world, SwanEntity swan) {
    ...
    public static Predicate<ItemStack> getTemptItemPredicate() {
        return stack -> stack.is(TagKey.create(Registries.ITEM, Identifier.fromNamespaceAndPath(OfBeastsAndWildThings.MOD_ID, "swan_food")));
    }
}
