...
public class BlocksWT {
    ...
    public static final Block BIRD_NEST = registerBlock("bird_nest",
            BirdNest::new, BlockBehaviour.Properties.ofFullCopy(Blocks.OAK_PLANKS).noOcclusion().noCollision(), ItemGroupsWT.BLOCKS_CONTENTS);

    public static Block registerBlock(String name, Function<BlockBehaviour.Properties, Block> factory, BlockBehaviour.Properties settings, List<net.minecraft.world.level.ItemLike> group){
        Block block = factory.apply(settings.setId(keyOfBlock(name)));
    ...
    static void registerBlockItem(String name, Block block) {
        var item =  Registry.register(BuiltInRegistries.ITEM, OfBeastsAndWildThings.of(name),
